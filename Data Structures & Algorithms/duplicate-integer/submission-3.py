    ...
    def hasDuplicate(self, nums: List[int]) -> bool:
      ...
      # for i in range (0, len(nums) - 1): #Do len(nums) - 1 because when we get to the next line we will already get to the last index 
       ...
       for i in range (0, len(nums)): #loop through the list 
        if nums[i] not in seen: #If the value has not been seen before you can add it to the hash set bc a hash set does not take duplicates. 
            seen.add(nums[i]) #If you do not have a duplicate then you can go ahead and add it. 
        else: #otherwise 
            return True #You can return true meaning that there is a duplicate 
       return False #If you loop through the array or list without finding duplicates, you can return false. 
